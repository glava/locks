...
trait StaticResourceSupport {
  ...
  def getResource(path: String, contentType: String, resourcePath: Option[String] = None): Future[Response] =
    readerResponse(classPathResource(resourcePath.getOrElse(path)), contentType)
}
